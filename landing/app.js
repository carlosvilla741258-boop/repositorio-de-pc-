...
/* Entrar en un tramo: saltar a su inicio y dejarlo correr.
   Con movimiento reducido no se arranca nada solo, pero el video se queda:
   se salta al fotograma final del tramo, que es justo el resultado del que
   habla la tarjeta, y quien quiera verlo moverse le da al play. */
function irA(i){
  if (i === cap) return;
  cap = i;
  tarjeta(i);
  if (!listo) return;
  if (reduce){ try { v.currentTime = CAPS[i].fin; } catch(e){} return; }
...
/* Que no haya video es lo único que deja la página sin nada que enseñar.
   Con <source> el fallo no llega al propio <video>: el evento se dispara en
   la última fuente y `v.error` se queda en null, así que hay que escucharlo
   en captura y mirar `networkState`, que es lo que de verdad dice si quedó
   alguna fuente en pie. */
function falloVideo(){
  if (document.documentElement.classList.contains("estatico")) return;
  if (v.networkState !== 3 && !v.error) return;   /* 3 = NETWORK_NO_SOURCE */
  const e = v.error;
  aEstatico("El video no pudo cargarse" + (e ? " (error " + e.code + ")" : "") + ".");
}
v.addEventListener("error", falloVideo);
v.addEventListener("error", falloVideo, true);
/* Y por lo mismo que los metadatos: con un 404 las fuentes fallan mientras la
   página se está leyendo, antes de que exista ningún oyente. Se comprueba a
   mano ahora y otra vez cuando termina de cargar todo. */
falloVideo();
addEventListener("load", falloVideo);

tarjeta(0);
ejeFlechas();
/* Con movimiento reducido se muestran los mandos del navegador: es la única
   forma visible de arrancarlo, ya que aquí no arranca solo. */
